...
namespace ComplSysSecurity_lab1
{
    public partial class Form1 : Form
    {
        ...
        private string decodeText(string givenText, string key)
        {
            ...
            for (int i = 0; i < givenText.Length; i++)
            {
                int newIndex = (getCharPos(txtArr[i]).Item1 - getCharPos(keyArr[i]).Item1 + alphabet.Length) % alphabet.Length;
                char newChar = getCharForIndex(newIndex);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            codedText = codeText(codedText, editKey(key, text.Length));
        ...
        {
            string key = keyTXT.Text;
            string text = decodeText(codedText, editKey(key, codedText.Length));
            decodedTXT.Text = decodeText(text, editKey(key, codedText.Length));
        }
    }
}
